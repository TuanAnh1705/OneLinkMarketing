...
// Per-platform SVG icons — matched to Strapi footer `socials[].platform`.
...
// Fallbacks — used only when Strapi footer is empty/unreachable.
const FALLBACK_SOCIALS = [
    { platform: "instagram", url: "https://www.instagram.com/onelink_marketing/" },
    { platform: "facebook", url: "https://www.facebook.com/profile.php?id=61582703650572" },
    { platform: "linkedin", url: "https://www.linkedin.com/company/onelink-marketing/" },
    { platform: "behance", url: "https://www.behance.net/onelinkmarketi" },
]
...
export default function Footer({ data }: { data?: FooterData | null }) {
    ...
    // Strapi-backed values with safe fallbacks so the footer always renders.
    const footerLogo = data?.logo?.url ? getMediaUrl(data.logo.url) : "/assets/logo.png"
    const socials = data?.socials?.length ? data.socials : FALLBACK_SOCIALS
    const companyTitle = data?.companyTitle || "Company"
    const servicesTitle = data?.servicesTitle || "Services"
    const companyLinks = data?.companyLinks?.length ? data.companyLinks : FALLBACK_COMPANY_LINKS
    const servicesLinks = data?.serviceLinks?.length ? data.serviceLinks : FALLBACK_SERVICE_LINKS
    const ctaLabel = data?.ctaLabel || "Let's Talk"
    const ctaHref = data?.ctaHref || "/contact"
    const newsletterPlaceholder = data?.newsletterPlaceholder || "Enter your email"
    const privacyLabel = data?.privacyLabel || "Privacy Policy"
    const privacyHref = data?.privacyHref || "/privacy-policy"
    ...
    return (
        <footer className="text-gray-800 -mt-40 md:mt-0 pt-20 pb-10 px-4 md:px-8 relative overflow-hidden bg-white">
                        ...
                        <div className="w-full max-w-125 mx-auto lg:mx-0 flex flex-col items-center lg:items-start text-center lg:text-left">
                            ...
                            <div className="flex justify-center lg:justify-start gap-5 mt-6 w-full">
                                {socials.map((s) => {
                                    ...
                                    if (!icon) return null
                                    return (
                                        <Link key={key} href={s.url} target="_blank" rel="noopener noreferrer" className="opacity-70 hover:opacity-100 transition-opacity">
                                            {icon}
                                        </Link>
                                    )
                                })}
                            ...
                            <Link href={privacyHref} className="generalsans-regular mt-8 text-[#000A1D] underline text-center lg:text-left w-full hover:opacity-70 transition-opacity">
                                {privacyLabel}
                            </Link>
                    ...
                    <div className="flex flex-col lg:items-end text-center lg:text-left">
                        ...
                        <div className="w-full max-w-md lg:self-end flex flex-col items-center lg:items-start">
                            ...
                            <form onSubmit={handleSubscribe} className="relative w-full">
                                ...
                                <Button
                                    ...
                                    variant="ghost"
                                    disabled={isLoading}
                                    className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full border border-none w-10 h-10 flex items-center justify-center hover:bg-transparent"
                                >
                                    {isLoading ? (
                        ...
                        <div className="flex flex-col md:flex-row items-start md:items-start justify-between gap-10 mt-14 w-full max-w-md lg:self-end">
                            <div className="text-left md:text-left flex-1">
                                <h4 className="generalsans-regular text-xl mb-4 text-[#444444]">{companyTitle}</h4>
                            ...
                            <div className="text-center md:text-left flex-1 flex flex-col">
                                <h4 className="generalsans-regular text-xl mb-4 text-[#444444] text-left md:text-left">{servicesTitle}</h4>
                                <ul className="generalsans-regular space-y-2 text-[#444444]">
            ...
            <div className="absolute bottom-5 left-0 w-full h-px bg-linear-to-r from-[#0074E5] to-[#162660]" />
